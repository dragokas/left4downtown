...
#define ACT_HULK_THROW 1255
#define ACT_HULK_ATTACK 1282

namespace Detours
{
	int SelectWeightedSequence::OnSelectWeightedSequence(int Activity)
	{
		//L4D_ANIM_PRINT("CBaseAnimating::SelectWeightedSequence(%d) has been called", Activity);
		cell_t result = Pl_Continue;
		int actualSequence;
		actualSequence = (this->*(GetTrampoline()))(Activity);

		if ((Activity != ACT_HULK_THROW) && (Activity != ACT_HULK_ATTACK))
		...
		if (g_pFwdOnSelectTankAttack)
		{
			...
			L4D_DEBUG_LOG("L4D_OnSelectTankAttack(client %d, sequence %d) forward has been sent out", client, overrideSequence);
